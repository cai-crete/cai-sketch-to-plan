...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const finalPrompt = `# System Prompt: Sketch-to-Plan Transformation System
...
* **Connectivity Mapping:** 버블들이 서로 맞닿아 있는 '인접면(Adjacency)'을 파악하고, 이를 '문(Door)'이나 '개구부(Opening)'가 필요한 연결 지점으로 해석합니다. 이때 한 벽에 여러 개의 문이 생성되지 않도록 파악하여, 한 벽에는 오직 한 개의 문만 생성되도록 제한합니다.
...
* **Graphic Bar Scale (그래픽 스케일 바 적용):** 치수와 스케일의 절대 기준은 그리드(Grid)입니다. 평면도 이미지와 스케일 바를 생성할 때 반드시 그리드를 기준으로 비례를 맞추며, 직관적인 공간 비례 확인을 위해 도면 내에 그래픽 스케일 바를 표기합니다.
